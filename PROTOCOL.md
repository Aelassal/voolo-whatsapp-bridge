...
- *(rev 2, feature `voice_waveform`)* `waveform?`: exactly 64 integers 0–100, the loudness bars WhatsApp's apps draw on a voice note; only with `kind: voice` (else `bad_request`). The duration is still read from the file.
