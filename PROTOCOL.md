...
### 6.1 `init`
```
{storeKey, storeDir, mediaDir, deviceName, limits: {historyDays, historyMaxPerChat, imageMaxBytes, voiceMaxSeconds, voiceMaxBytes?, videoMaxBytes?, fileMaxBytes?}}
```
...
- *(rev 2)* `videoMaxBytes?` 1–67,108,864 (absent: 16,777,216) caps videos sent and fetched; `fileMaxBytes?` 1–104,857,600 (absent: 33,554,432, the first release's fixed cap for `kind: file`) caps documents sent and fetched, and received audio files. The ceilings are 64 MiB and 100 MiB.
...
### 6.6 `send_media`
`{chatJid, outboxId, kind: "image"|"voice"|"file"|"video", path, key, sha256, mime, caption?, fileName?, durationS?, width?, height?, waveform?, quotedMessageId?, quotedSenderJid?, quotedText?, forwarded?}`. The client writes the file in the §8 format under `mediaDir` with its own fresh key. The bridge reads, decrypts and checks it, uploads it, deletes the file, and replies. Same rules and errors as §6.5, plus `media_too_large` and `media_invalid`.
- *(clarified)* `path` must be `<mediaDir>/<32 lowercase hex>.bin`, a regular file (not a link). It is deleted once read, in every case. *(review)* Also when the command is refused before the file is read (`busy`, `unknown_chat`, `duplicate_outbox_id`, `not_connected`, …). The file is opened once and checked on the open handle, so a path swapped for a link after the check, or a file cut short, gets `media_invalid`. Caps: `image` ≤ `limits.imageMaxBytes`; `voice` ≤ `limits.voiceMaxBytes` and ≤ `limits.voiceMaxSeconds` (read from the file); `file` ≤ 33,554,432 bytes *(rev 2: ≤ `limits.fileMaxBytes`)*; `video` ≤ `limits.videoMaxBytes` *(rev 2)*. *(review)* A `voice` must be Ogg Opus (`mime` `audio/ogg`, with or without parameters) whose duration the bridge can read; anything else is `media_invalid`. *(re-review)* The duration is read from a check of every page of the file: one Opus stream that starts with its `OpusHead`, pages that follow each other without gaps or trailing bytes, sequence numbers that increase by one, granule positions that never decrease, nothing after the end-of-stream page. A file that fails any of this is `media_invalid`. The duration is the largest granule minus the pre-skip, and never less than the file's size divided by 72,000 bytes per second (Opus's highest bitrate plus the Ogg framing). `voice` is sent as a voice note (push-to-talk). `mime`: `type/subtype` with optional parameters, for example `audio/ogg; codecs=opus`.
...
- *(rev 2, feature `send_video`)* `kind: video` sends a video message (`mime` MUST be `video/*`, else `media_invalid`), with the `caption`. `durationS?` (0–86,400), `width?` and `height?` (1–16,384 each) are what the recipient's app shows before it downloads; they are the client's word and only allowed with `kind: video` (else `bad_request`). No thumbnail is sent in revision 2.
- *(rev 2, feature `voice_waveform`)* `waveform?`: the 64 loudness bars WhatsApp's apps draw on a voice note, each 0–100, as **128 lowercase hex characters** (one byte per bar; a string, so that a send carries no list of anything); only with `kind: voice` (else `bad_request`). The duration is still read from the file.
...
- *(clarified)* v1 fetches only `voice` and `image` messages (owner decision: video, documents, stickers and other audio stay on the phone). For any other kind the bridge keeps no download descriptor, so `fetch_media` answers `unknown_message`.
- *(rev 2, feature `fetch_all_media`; changes the line above)* `video`, `sticker`, `document` and `audio` messages are fetched too. Caps: `image` and `sticker` ≤ `imageMaxBytes`; `voice` as before; `video` ≤ `videoMaxBytes`; `document` and `audio` ≤ `fileMaxBytes` (audio files have no duration cap, only the size). The bridge keeps download descriptors for these kinds from this revision on: a message of these kinds reported by an older bridge has none and still gets `unknown_message`. Location, contact, poll, system and unsupported messages have nothing to fetch (`unknown_message`). `chatJid` is the JID under which the message was reported, or the phone-number JID of a chat merged through `aliasOf`. At most two downloads run at once; more wait.
...
2. It writes the file to `<mediaDir>/<random 32 hex>.bin` in this format: `nonce (12 bytes) ‖ ciphertext ‖ tag (16 bytes)`, AES-256-GCM, with a **fresh random 32-byte key** for this file and no additional data. Caps (§6.1): images ≤ 16 MiB, voice notes ≤ 60 minutes and ≤ 32 MiB, *(rev 2)* videos ≤ `videoMaxBytes` (≤ 64 MiB), documents and audio files ≤ `fileMaxBytes` (≤ 100 MiB), checked against the description before downloading and against the real size after.
