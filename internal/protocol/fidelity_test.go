...
// Revision 2, feature voice_waveform: exactly 64 bars of 0–100 as 128 hex
// characters, voice only.
func TestWaveform(t *testing.T) {
	bars := func(n, v int) string {
		return `"` + strings.Repeat(fmt.Sprintf("%02x", v&0xff), n) + `"`
	}
	...
	for payload, ok := range map[string]bool{
		...
		media("voice", bars(64, 255)):                    false,
		media("file", bars(64, 5)):                       false,
		media("voice", `[1,2]`):                          false,
		media("voice", `"`+strings.Repeat("0A", 64)+`"`): false, // lower-case hex only
		media("voice", `""`):                             true,  // empty is absent
	} {
